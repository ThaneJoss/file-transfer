...
1. 使用 Passkey 登录。
2. 选择文件和模式后，页面先在 Pickup Durable Object 中预留 8 位取件码并立即显示。
3. 浏览器在后台分块计算 SHA-256，并行准备五条线路；此时只会发送信令和小型测速对象，不发送文件正文。
4. 准备完成后，发送方把 `file-transfer-v3` 多路协议一次性发布到已预留的取件码。协议包含文件清单和可用线路描述，不包含 R2 临时 secret access key。
5. 接收方可以立即输入取件码；如果 Offer 仍在准备，读取请求会等待 DO 状态变为 ready，随后双方建立可用线路并做端到端吞吐探测。
6. 智能模式根据连接时间和实测吞吐估算完成时间，只让预计最快的线路发送正文；失败时继续尝试其他可用线路。
7. 极速模式让所有可用实时线路发送相同的有序分块，同时启动 R2 上传；接收端按 sequence 去重，只写入一份文件，首个通过大小和 SHA-256 校验的结果获胜并取消其他任务。
...
- 前端 `file-transfer-v3` 必须与支持两阶段 Pickup Offer、`multipath`、384 KiB 信令以及 selection / winner 接口的 API 版本一起部署；API 应先于前端上线。
