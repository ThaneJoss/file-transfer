...
1. 发送方使用 Passkey 登录，选择文件、智能或极速模式，以及是否启用端到端加密。
2. 页面先在 Pickup Durable Object 中预留 8 位取件码并立即显示，文件哈希与五条线路在后台并行准备。
3. 默认发布 `file-transfer-v4` 协议：文件正文按分块使用 AES-256-GCM 加密，密钥只放在分享 URL 的 `#key=` 片段中，不会进入 HTTP 请求、取件码服务或诊断日志。关闭加密时继续发布兼容的 `file-transfer-v3`。
...
旧 `file-transfer-v2` R2 取件码和未加密 `file-transfer-v3` 取件码仍可接收。

## 大文件、恢复与取消

- 小文件在主线程以 4 MiB 读取块增量计算 SHA-256；8 MiB 及以上文件交给独立 Web Worker，避免哈希阻塞交互。
- 实时传输使用 48 KiB 有序分块、DataChannel 背压和分块确认。AES-GCM 为每个 sequence 派生独立 nonce，并为每块附加 16 字节认证标签。
- 未加密且小于 8 MiB 的 R2 文件使用单次 PUT；加密文件和 8 MiB 及以上文件使用 S3 兼容 multipart 上传。
- multipart 的 `uploadId`、已完成 part/ETag、对象 key 与文件指纹会在当前设备保留 24 小时。重试时 API 只会为当前用户拥有的原对象重新签发短期凭据，已完成分块不会重复上传。
- 加密恢复会复用同一文件的临时密钥；传输成功后同时清理密钥和 multipart 恢复记录。
...
- `src/features/transfer/protocol/fileProtocol.ts`：v2/v3/v4 协议和严格运行时校验。
- `src/features/transfer/crypto/fileEncryption.ts`：AES-GCM 密钥、分块 nonce、分享片段及恢复密钥。
...
- `src/features/transfer/services/channelTransfer.ts`：实时加密分块、背压、确认和接收完整性。
...
`pnpm check` 串行执行全部检查。单元测试覆盖 v4 加密、Worker 哈希、multipart 恢复、长轮询客户端、路由超时、极速去重、完整性和取消。Playwright 除常规 API / R2 mock 场景外，还会在两个真实 Chromium 页面之间使用原生 `RTCPeerConnection` 完成一次加密传输，用来捕获真实浏览器对象序列化和 DataChannel 编排问题。
...
- 先部署 API 与 D1 migration `0006_guest_claim_rate_limit.sql`，再部署发布 v4 分享链接的前端。
