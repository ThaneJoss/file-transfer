    ...
    <div className="grid min-h-0 flex-1 grid-cols-[minmax(340px,0.92fr)_minmax(0,1.85fr)] grid-rows-[auto_minmax(180px,0.5fr)] gap-[clamp(12px,1.2vw,18px)] max-[1180px]:grid-cols-1 max-[1180px]:grid-rows-none max-[1180px]:gap-[clamp(14px,1.5vw,22px)]">
      <Panel className="row-span-2 flex min-h-0 flex-col overflow-hidden p-[clamp(16px,1.45vw,22px)] max-[1180px]:row-span-1 max-[1180px]:overflow-visible max-[1180px]:p-[clamp(18px,1.8vw,28px)]">
        ...
        <div className="relative grid shrink-0 grid-cols-4 items-start max-[620px]:grid-cols-1 max-[620px]:gap-5">
          <div className="absolute left-[12.5%] right-[12.5%] top-[26px] grid grid-cols-3 max-[620px]:hidden">
            {steps.slice(0, -1).map((step) => (
              <span className={`mx-7 h-[3px] rounded-full ${step.active ? "bg-[#1677ff]" : "bg-[#cdd8e7]"}`} key={`connector-${step.label}`} />
            ))}
          </div>
          {steps.map((step) => {
            ...
            return (
              <div className="relative z-10 grid min-w-0 justify-items-center text-center max-[620px]:grid-cols-[56px_1fr] max-[620px]:justify-items-start max-[620px]:gap-3 max-[620px]:text-left" key={step.label}>
                <span className={`grid size-[54px] place-items-center rounded-2xl text-white shadow-[0_10px_25px_rgba(47,125,246,0.25)] ${step.active ? "bg-[#1677ff]" : "bg-[#aeb8c8]"}`}>
        ...
        <div className="grid shrink-0 grid-cols-2 gap-2.5 max-[560px]:grid-cols-1">
          <Metric label="连接类型" value="Cloudflare SFU DataChannel" icon={Link2} />
          <Metric label="API" value="rtc.live.cloudflare.com/v1" icon={Server} />
          <Metric label="发布 Session" value={publisherSessionId || "未创建"} icon={UploadCloud} active={Boolean(publisherSessionId)} />
          <Metric label="订阅 Session" value={subscriberSessionId || "未创建"} icon={Download} active={Boolean(subscriberSessionId)} />
          <Metric label="发送端状态" value={`${senderPeerState} / ${senderIceState}`} icon={Circle} active={senderPeerState === "connected"} />
          <Metric label="接收端状态" value={`${receiverPeerState} / ${receiverIceState}`} icon={Circle} active={receiverPeerState === "connected"} />
          <Metric label="发送通道" value={senderChannelState} icon={Wifi} active={senderReady} />
          <Metric label="接收通道" value={receiverChannelState} icon={Wifi} active={receiverReady} />
          <Metric label="DataChannel" value={dataChannelName || "未注册"} icon={FileText} />
          <Metric label="选中文件" value={selectedFile ? selectedFile.name : incomingMeta?.name ?? "未选择"} icon={HardDrive} />
          <Metric label="文件大小" value={totalBytes ? formatBytes(totalBytes) : "0 B"} icon={Database} />
          <Metric label="已发送" value={formatBytes(sentBytes)} icon={UploadCloud} />
          <Metric label="已接收" value={formatBytes(receivedBytes)} icon={Download} />
      ...
      <div className="grid min-h-0 grid-cols-2 gap-[clamp(12px,1.2vw,18px)] max-[980px]:grid-cols-1 max-[980px]:gap-[clamp(14px,1.5vw,22px)]">
          ...
          {!mode && (
            <div className="grid gap-3">
              ...
              >
                ...
                <span className="min-w-0">
                  <strong className="block text-[15px] font-extrabold text-[#071b3a]">发送文件</strong>
                  <span className="block truncate text-[13px] text-[#526c92]">创建发布通道并复制连接码</span>
                </span>
              </button>
              <button
                className="grid min-h-[68px] grid-cols-[22px_34px_minmax(0,1fr)] items-center gap-3 rounded-xl border border-[#d7e5f6] bg-white/80 px-3 text-left transition hover:-translate-y-px hover:border-[#1677ff] hover:bg-white"
              ...
              >
                ...
                <span className="min-w-0">
                  <strong className="block text-[15px] font-extrabold text-[#071b3a]">接收文件</strong>
                  <span className="block truncate text-[13px] text-[#526c92]">粘贴连接码并订阅 SFU DataChannel</span>
                </span>
              </button>
            </div>
          )}
        ...
        <Panel className="min-h-0 overflow-hidden p-[clamp(16px,1.45vw,22px)] max-[1180px]:overflow-visible max-[1180px]:p-[clamp(18px,1.8vw,28px)]">
          ...
          >
            ...
            <span className="mt-1 text-[14px] text-[#526c92]">{selectedFile ? formatBytes(selectedFile.size) : "发送端选择文件后创建 SFU 发布通道"}</span>
      ...
      <Panel className="flex min-h-0 flex-col overflow-hidden p-[clamp(16px,1.45vw,22px)] max-[1180px]:overflow-visible max-[1180px]:p-[clamp(18px,1.8vw,28px)]">
        ...
        <div className={`grid min-h-0 gap-3 ${receivedFiles.length > 0 ? "overflow-auto pr-1" : "overflow-hidden"}`} role="table" aria-label="已接收文件列表">
